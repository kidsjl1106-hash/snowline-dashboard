...
function login_(payload) {
  ...
  if (!found.user) throw new Error("계정 또는 비밀번호가 올바르지 않습니다.");

  const user = found.user;
  if (user.status !== "approved") throw new Error("관리자 승인 후 로그인할 수 있습니다.");
  if (hashPassword_(password, user.passwordSalt) !== user.passwordHash) {
    audit_("login_failed", userId, "bad_password");
